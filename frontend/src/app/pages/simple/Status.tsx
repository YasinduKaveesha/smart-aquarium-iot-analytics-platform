...
import { Thermometer, Droplets, Zap, Eye, TrendingUp, TrendingDown, Minus, CheckCircle, AlertCircle, Wifi } from 'lucide-react';
...
export function SimpleStatus() {
  ...
  const sensors = [
    ...
    },
    {
      ...
      displayValue: phFailed ? '--' : live.pH.toFixed(2),
      color: '#2E75B6', min: 6.5, max: 7.5, range: '6.5–7.5',
      trend: phFailed ? { icon: Wifi, color: '#EF4444', label: 'FAULT' } : getTrend(live.pH, prev.pH),
      chartKey: 'pH' as const,
      ok: phFailed ? false : (live.pH >= 6.5 && live.pH <= 7.5),
      normalized: phFailed ? 0 : Math.min(Math.max((live.pH - 6.5) / (7.5 - 6.5), 0), 1),
      failed: phFailed,
    },
    {
      icon: Zap, label: 'TDS',
    ...
    <SimpleLayout>
      <div className="px-4 pt-5 pb-28 lg:pb-12 lg:px-10">
        <div className="flex items-center gap-4 mb-5">
          ...
          <div className="ml-auto flex items-center gap-3 px-4 py-2.5 rounded-2xl" style={{ background: isSensorError ? '#FEF2F2' : wqiInfo.bgColor, border: `1.5px solid ${isSensorError ? '#EF444430' : `${wqiInfo.color}30`}` }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-white text-sm shadow-sm flex-shrink-0" style={{ background: isSensorError ? '#EF4444' : wqiInfo.color }}>
              {isSensorError ? 'N/A' : live.wqi}
            </div>
            <div>
              <p className="text-xs font-bold text-[#1A3D5C] leading-none">Water Quality</p>
              <p className="text-sm font-black leading-tight" style={{ color: isSensorError ? '#EF4444' : wqiInfo.color }}>{isSensorError ? 'Unavailable' : wqiInfo.status}</p>
            </div>
            <div className="ml-2">
              {isSensorError
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          {sensors.map(({ icon: Icon, label, displayValue, color, range, trend, chartKey, ok, normalized, failed }) => {
            const TrendIcon = trend.icon;
            const borderColor = failed ? '#FEE2E2' : ok ? '#F0FDF4' : '#FFF7ED';
            return (
              <div key={label} className="bg-white rounded-2xl p-4 relative overflow-hidden flex flex-col" style={{ border: `1.5px solid ${borderColor}`, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                <div className="absolute -top-5 -right-5 w-14 h-14 rounded-full pointer-events-none" style={{ background: `radial-gradient(circle, ${failed ? '#EF4444' : color}, transparent)`, opacity: 0.07 }} />
                <div className="flex items-center justify-between mb-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: failed ? '#FEE2E212' : `${color}12` }}>
                    <Icon className="w-5 h-5" style={{ color: failed ? '#EF4444' : color }} />
                  </div>
                  {failed
                ...
                {failed ? (
                  <>
                    <p className="text-2xl font-black text-red-500 leading-tight mb-1">FAULT</p>
                    <div className="flex items-center gap-1 mb-3">
                      <Wifi className="w-3 h-3 text-red-400" />
                      <span className="text-[11px] font-semibold text-red-400">Sensor not connected</span>
                    </div>
                    <div className="h-1.5 rounded-full bg-red-50 overflow-hidden mb-1">
                      <div className="h-full rounded-full bg-red-300 animate-pulse" style={{ width: '100%' }} />
                    </div>
                    <p className="text-[9px] text-red-400 mb-2">Sensor failure · No data</p>
                    <div className="h-12 -mx-1 mt-auto flex items-center justify-center">
                      <p className="text-[10px] text-red-300 font-medium">No chart data</p>
                    </div>
                  </>
                ) : (
                  <>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm" style={{ border: '1.5px solid #F1F5F9' }}>
          ...
          <div className="space-y-2">
            {phFailed && (
              <div className="flex items-start gap-2 px-3 py-2 rounded-xl" style={{ background: '#FEF2F2', border: '1px solid #FECACA' }}>
                <Wifi className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" />
                <p className="text-xs text-red-700">
                  <span className="font-bold">pH sensor not connected.</span> WQI prediction is unavailable. Check sensor wiring and reconnect.
                </p>
              </div>
            )}
            {sensors.filter(s => !s.ok && !s.failed).length === 0 && !phFailed ? (
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0" />
                <p className="text-sm text-[#374151]">All parameters are within optimal range. Keep up the current schedule.</p>
              </div>
            ) : (
              sensors.filter(s => !s.ok && !s.failed).map(s => (
